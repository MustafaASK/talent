...
export class ApplyJobsResumeComponent implements OnInit {
  jobsTab = true;
  jobForm: FormGroup;
  jobsList: Job[] = [];
  jobtypes: JobType[] = [];
...
// debugger
    ...
    formData.append(
      'status',
      this.auditlogService.encryptAES(this.selectedJob.customAttributes_.mapData.status ? this.selectedJob.customAttributes_.mapData.status.stringValues_[0] : '')
    );
    formData.append('accuickJobId', this.selectedJob.requisitionId_);
    formData.append(
      'jobTitle',
      this.auditlogService.encryptAES(this.selectedJob.title_)
    );
    formData.append(
      'clientName',
    ...
    formData.append(
      'city',
      this.auditlogService.encryptAES(this.selectedJob.addresses_[0])
    );
    formData.append(
      'state',''
      // this.auditlogService.encryptAES(this.selectedJob.state)
    ...
    formData.append(
      'description',
      this.auditlogService.encryptAES(this.selectedJob.description_)
    );
    formData.append(
      'phoneNo',''
    );
    formData.append('userType', (localStorage.getItem('userTypeToken') || ''));
      ...
      (response) => {
        ...
        if(response.body.Success){
          if(response.body.jobAlreadyApplied){
            if((response.body.userId || response.body.userType) && response.body.firstName){
  ...
  submitForm(event: any) {
    ...
    this.userService.applyJobCareer(formData).subscribe(
      (response) => {
        ...
        if(response.body.Success){
          this.userService.setUserId(response.body.userId);
          this.userService.setUserType(this.loginForm.value.userType ? "true" : "false");
          localStorage.setItem('firstName', response.body.firstName);
          localStorage.setItem('lastName', response.body.lastName);
          localStorage.setItem('email', response.body.email);
  ...
  getcareerjobdetails() {
    ...
    this.userService.getcareerjobdetails(datatopass).subscribe(
      (response: any) => {
        this.showSpinner = false;
        console.log(response);
        if (response && response.Status == 401) {
        ...
        if (response && response.Success) {
          ...
          localStorage.setItem('jobData', JSON.stringify(response.JobDetails[0]));
          this.selectedJob = response.JobDetails[0];
