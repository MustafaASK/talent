...
} from '../common-component/modals/delete-confirm-dialog/delete-confirm-dialog.component';
...
@Component({
  selector: 'app-verified-email-continue-apply',
  templateUrl: './verified-email-continue-apply.component.html',
  styleUrls: ['./verified-email-continue-apply.component.css'],
})
export class VerifiedEmailContinueApplyComponent implements OnInit {
  CKEDITOR: any;
  ...
  isResumeBuildeBasicinfopage = false;
  /*****  Summary */
  ...
  @ViewChild('contactInfo', { static: false }) contactInfoToChange: any;
  color1 = 'primary';
  loginForm: any;
  submitted = false;
  ...
  getLanguageList() {
    this.userService.getlanguageslist().subscribe(
      (response: any) => {
        if (response && response.Status) {
...
// document.documentElement.style.setProperty('--vh', `${vh}px`);
    ...
    this.getjobData();
    this.file = this.userService.resume;
  }
        ...
        if (response && response.Success) {
          ...
          for (let e = 0; e < this.employmentList.length; e++) {
            if (this.employmentList[e].currentCompany == true) {
              ...
              this.eventEndTime = moment(this.employmentList[e].endDate).format(
                'YYYY-MM-DD'
              );
            }

            let eventStartTime = moment(
              this.employmentList[e].startDate
            ).format('YYYY-MM-DD');
            let m = moment(this.eventEndTime);
            let years =
              m.diff(eventStartTime, 'years') != 0
                ? m.diff(eventStartTime, 'years')
                : '';
            m.add(-years, 'years');
            let months = m.diff(eventStartTime, 'months');
            //m.add(-months, 'months');
            //let days = m.diff(eventStartTime, 'days');
            this.employmentList[e].countofYearsMonths =
              (years != '' ? years + ' ' + 'Yrs' : '') +
              ' ' +
              months +
              ' ' +
          ...
          for (let ed = 0; ed < this.educationList.length; ed++) {
            if (
              this.educationList[ed].degreeCompletionDate != null &&
              this.educationList[ed].degreeCompletionDate.indexOf('1900') != -1
            ) {
          ...
          for (let t = 0; t < this.trainingList.length; t++) {
            if (
              this.trainingList[t].completedYear != null &&
              this.trainingList[t].completedYear.indexOf('1900') != -1
            ) {
          ...
          //*** contact InfoList */
          this.editedBasicInfoRecordData =
            response.contactInfoList && response.contactInfoList.length
              ? response.contactInfoList[0]
          ...
          if (response.summary && response.summary.length) {
            ...
            this.dataDecryption = response.summary['0'].summaryDesc
              ? this.removeJunk.addBreaks(
                  this.auditlogService.decryptAES(
                    response.summary['0'].summaryDesc
                  )
                )
              : '';
            this.summaryDesc = this.dataDecryption;
          }
          // .replace(/<br class=\"manual\"><br class=\"manual\">/g, '<br class="manual">➢')
          ...
          for (let ct = 0; ct < this.certficationList.length; ct++) {
            if (
              this.certficationList[ct].completedYear != null &&
              this.certficationList[ct].completedYear.indexOf('1900') != -1
            ) {
          ...
          this.candData.name =
            this.auditlogService.decryptAES(
              response.contactInfoList[0].firstName
            ) +
            ' ' +
            this.auditlogService.decryptAES(
              response.contactInfoList[0].lastName
            );

          let location = [];
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].address)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(
                response.contactInfoList[0].address
              )
            );
          }
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].apt)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(response.contactInfoList[0].apt)
            );
          }
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].cityName)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(
                response.contactInfoList[0].cityName
              )
            );
          }
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].stateName)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(
                response.contactInfoList[0].stateName
              )
            );
          }
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].countryName)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(
                response.contactInfoList[0].countryName
              )
            );
          }
          if (
            this.auditlogService
              .decryptAES(response.contactInfoList[0].zipcode)
              .trim()
          ) {
            location.push(
              this.auditlogService.decryptAES(
                response.contactInfoList[0].zipcode
              )
            );
          }
          ...
          if (response.resumePath && response.resumePath.length) {
            ...
            this.resumeData.path =
              environment.amazonS3 +
              'Sevron/' +
              response.resumePath[0].resumeID;
            this.resumeData.pdfDown = response.resumePath[0].resumeTOPdfPath
              ? environment.amazonS3 +
                'Sevron/' +
                localStorage.getItem('userId') +
                '/' +
                response.resumePath[0].resumeTOPdfPath
              : '';
          }

          this.candData.firstName = this.auditlogService.decryptAES(
            response.contactInfoList[0].firstName
          );
          this.candData.lastName = this.auditlogService.decryptAES(
            response.contactInfoList[0].lastName
          );
          this.candData.jobTitle = this.auditlogService.decryptAES(
            response.contactInfoList[0].jobTitle
          );
          this.candData.email = this.auditlogService.decryptAES(
            response.contactInfoList[0].email
          );
          this.candData.phoneNo = this.auditlogService.decryptAES(
            response.contactInfoList[0].phoneNo
          );
          this.candData.cityName = this.auditlogService.decryptAES(
            response.contactInfoList[0].cityName
          );
          this.candData.stateName = this.auditlogService.decryptAES(
            response.contactInfoList[0].stateName
          );
          this.candData.countryName = this.auditlogService.decryptAES(
            response.contactInfoList[0].countryName
          );
          this.candData.zipcode = this.auditlogService.decryptAES(
            response.contactInfoList[0].zipcode
          );
          this.employmentList.sort((a: any, b: any) => a.orderBy - b.orderBy);
          this.candData.empHistory = this.employmentList.map((obj: any) => ({
            ...obj,
          }));
          this.candData.education = this.educationList.map((obj: any) => ({
            ...obj,
          }));
          this.candData.skills = this.skillsList.map((obj: any) => ({
            ...obj,
          }));
          this.candData.socialsLinks = this.socialLinksList.map((obj: any) => ({
            ...obj,
          }));
          this.candData.languages = this.languageList.map((obj: any) => ({
            ...obj,
          }));
          this.candData.careersummary = this.summaryDesc;
          this.candData.certficationList = this.certficationList.map(
            (obj: any) => ({ ...obj })
          );

          this.candData.empHistory.forEach((ele: any) => {
            ele.jobTitle = ele.jobTitle
              ? this.auditlogService.decryptAES(ele.jobTitle)
              : '';
            ele.companyName = ele.companyName
              ? this.auditlogService.decryptAES(ele.companyName)
              : '';
            ele.workAddress = ele.workAddress
              ? this.auditlogService.decryptAES(ele.workAddress)
              : '';
            ele.empResponsibilities = ele.empResponsibilities
              ? this.auditlogService.decryptAES(
                  ele.empResponsibilities.replace(/\n/g, '').replace(/\r/g, '')
                )
              : '';
          ...
          this.candData.education.forEach((ele: any) => {
            ele.degreeName = ele.degreeName
              ? this.auditlogService.decryptAES(ele.degreeName)
              : '';
            ele.degreeType = ele.degreeType
              ? this.auditlogService.decryptAES(ele.degreeType)
              : '';
            ele.schoolName = ele.schoolName
              ? this.auditlogService.decryptAES(ele.schoolName)
              : '';
            // degreeCompletionDate: "2010-01-01 00:00:00.0"
          });
          let skillsObj = this.userService.categoryList.filter(function (
            obj: any
          ) {
          ...
          let trainingsObj = this.userService.categoryList.filter(function (
            obj: any
          ) {
          ...
          this.candData.trainingList.forEach((ele: any) => {
            ele.trainingName = ele.trainingName
              ? this.auditlogService.decryptAES(ele.trainingName)
              : '';
            ele.institutionName = ele.institutionName
              ? this.auditlogService.decryptAES(ele.institutionName)
              : '';
            ele.completedYear = this.userService.getDateFormat(
              ele.completedYear
            );
            let tempObj = trainingsObj[0].lookupsList.filter(function (
              obj: any
            ) {
          ...
          let socialsObj = this.userService.categoryList.filter(function (
            obj: any
          ) {
          ...
          let langsObj = this.userService.categoryList.filter(function (
            obj: any
          ) {
          ...
          let certsObj = this.userService.categoryList.filter(function (
            obj: any
          ) {
            return obj.categoryID == 10005;
          });
          this.candData.certficationList.forEach((ele: any) => {
            ele.authorityName = ele.authorityName
              ? this.auditlogService.decryptAES(ele.authorityName)
              : '';
            ele.certName = ele.certName
              ? this.auditlogService.decryptAES(ele.certName)
              : '';
            ele.authorityName = ele.authorityName
              ? this.auditlogService.decryptAES(ele.authorityName)
              : '';
          ...
          setTimeout(() => {
            this.contactInfoToChange.loaddata();
          }, 50);
          this.getPreferences();
        }
      },
      ...
      (response: any) => {
        ...
        if (response && response.Success) {
          if (response.preferenceList && response.preferenceList.length) {
            ...
            let milesListObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10014;
            });
            if (milesListObj && milesListObj.length) {
              milesListObj[0].lookupsList.forEach(
                (element: { lookupValue: any; lookupId: any }) => {
                  if (
                    element.lookupId ==
                    response.preferenceList[0].empRoleMilesLookupID
                  ) {
            ...
            this.candData.empHourCompensation =
              response.preferenceList[0].empHourCompensation;
            this.candData.empPrefLocation =
              response.preferenceList[0].empPrefLocation;
            this.candData.empPrefRoleTitle =
              response.preferenceList[0].empPrefRoleTitle;

            this.candData.requireVisa =
              response.preferenceList[0].visaSponsorStatus;
            this.candData.legalUS = response.preferenceList[0].legalStatus;
            this.candData.compensationPref =
              response.preferenceList[0].empYearCompensation;

            let availObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10011;
            });
            let availTempObj = availObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return (
                obj.lookupId == response.preferenceList[0].empAvailLookupID
              );
            });
            this.candData.availability = availTempObj.length
              ? availTempObj[0].lookupValue
              : '';

            let flexObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10013;
            });
            let flexTempObj = flexObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return obj.lookupId == response.preferenceList[0].empFlexLookupID;
            });
            this.candData.flexibilityPref = flexTempObj.length
              ? flexTempObj[0].lookupValue
              : '';

            let prefObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10012;
            });
            let prefTempObj = prefObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return obj.lookupId == response.preferenceList[0].empPrefLookupID;
            });
            this.candData.empPerf = prefTempObj.length
              ? prefTempObj[0].lookupValue
              : '';

            let relocationPrefObj = this.userService.categoryList.filter(
              function (obj: any) {
                return obj.categoryID == 10016;
              }
            );
            let relocationPrefTempObj = relocationPrefObj[0].lookupsList.filter(
              function (obj: any) {
                return (
                  obj.lookupId ==
                  response.preferenceList[0].empRelocPrefLookupID
                );
              }
            );
            this.candData.relocationPref = relocationPrefTempObj.length
              ? relocationPrefTempObj[0].lookupValue
              : '';

            let roleMilesObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10017;
            });
            let roleMilesTempObj = roleMilesObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return (
                obj.lookupId == response.preferenceList[0].empRoleMilesLookupID
              );
            });
            this.candData.miles = roleMilesTempObj.length
              ? roleMilesTempObj[0].lookupValue
              : '';

            let empStatusObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10010;
            });
            let empStatusTempObj = empStatusObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return (
                obj.lookupId == response.preferenceList[0].empStatusLookupID
              );
            });
            this.candData.currEmpStatus = empStatusTempObj.length
              ? empStatusTempObj[0].lookupValue
              : '';

            let travelPerObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10015;
            });
            let travelPerTempObj = travelPerObj[0].lookupsList.filter(function (
              obj: any
            ) {
              return (
                obj.lookupId ==
                response.preferenceList[0].empTravelPerTimeLookupID
              );
            });
            this.candData.travelPrefPercent = travelPerTempObj.length
              ? travelPerTempObj[0].lookupValue
              : '';

            let travelMilesObj = this.userService.categoryList.filter(function (
              obj: any
            ) {
              return obj.categoryID == 10014;
            });
            let travelMilesTempObj = travelMilesObj[0].lookupsList.filter(
              function (obj: any) {
                return (
                  obj.lookupId ==
                  response.preferenceList[0].empTravelMilesLookupID
                );
              }
            );
            this.candData.travelPrefMiles = travelMilesTempObj.length
              ? travelMilesTempObj[0].lookupValue
              : '';
  ...
  updateCandData(obj: any) {
    if (obj.name == 'basicInfo') {
      ...
      this.candData.careersummary = this.removeJunk.addBreaks(
        obj.careerSummary
      );
  ...
  saveHtml() {
    if (
      this.candData.firstName.trim() &&
      this.candData.lastName.trim() &&
      this.candData.email.trim()
    ) {
      ...
      this.userService.saveHtml(dataToPass, this.authToken).subscribe(
        (response: any) => {
          this.showSpinner = false;
          if (response && response.Status) {
            console.log(response);
  ...
  deleteEmpForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deleteemploymentrecord(id).subscribe(
          (response: any) => {
            // console.log(response);
            // let resp = JSON.parse(response);
            // console.log(response.Status);
  ...
  deleteEduForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deleteeducationrecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  deleteTrainForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deletetrainingrecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  deleteSLForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deletesociallinkrecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  deleteLanguageForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deletelanguagerecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  deleteLicenceCertificateForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deletecertificationrecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  deleteSkillForm(indx: any) {
    ...
    dialogRef.afterClosed().subscribe((dialogResult) => {
      ...
      if (this.deleteconfirmresult) {
        ...
        this.userService.deleteskillrecord(id).subscribe(
          (response: any) => {
            if (response && response.Status) {
  ...
  addSummary($event: Event) {
    ...
    this.dataDecryption = this.SummaryList[0].summaryDesc
      ? this.removeJunk.addBreaks(
          this.auditlogService.decryptAES(this.SummaryList[0].summaryDesc)
        )
      : '';
    this.summaryDesc = this.dataDecryption;
  }
  ...
  setNewEndDate(i: number) {
    ...
    this.startDateEmp = new Date(
      this.resumeBuilderFrom.controls.employees.value[i].startDate
    );
  ...
  checkDates(i: number) {
    if (
      this.resumeBuilderFrom.controls.employees.value[i].endDate <
      this.resumeBuilderFrom.controls.employees.value[i].startDate
    ) {
  ...
  empOrderChange(event: CdkDragDrop<string[]>) {
    ...
    // console.log(this.employmentList);
    let tempid = 1;
    let sortedEmpList = this.employmentList.map((choice: any) => ({
      userEmploymentId: choice.userEmploymentID,
      orderBy: tempid++,
    }));
  ...
  selectCheckBox(targetType: CheckBoxType) {
    if (this.currentlyChecked === targetType) {
      ...
      this.toastr.error('Please select at least one option to proceed');
      return;
    }
  ...
  submitForm(event: any, type: string) {
    ...
    if (this.file && this.file.length) {
      formData.append(
        'resume',
        this.file && this.file.length ? this.file[0] : ''
      );
    }
    formData.append(
      'source',
    ...
    formData.append('newResume', type);
    this.showSpinner = true;

    this.userService.continuetoapplyjob(formData).subscribe(
      (response) => {
        this.showSpinner = false;
        if (response && response.Success) {
