  ...
  loadData(obj: any) {
    ...
    this.candData.empHistory.forEach((el: any) => {
      ...
      this.empHistoryElements += `<li class='innerpadding emprecord' style='line-height: 30px;padding: 0 30px;'>
          <div class='companydata'>
              <h4 style='font-family: Jost, sans-serif;font-weight: normal;font-size: 22px;margin-bottom: 5px;'>${el.jobTitle} | ${el.companyName}</h4>
              <span class='subheading' style='color: #969EB5;'>${el.startDate.substring(0, 4)} - ${el.currentCompany ? 'Present' : el.endDate.substring(0, 4)} |
                  ${el.workAddress}</span>
          </div>
    ...
    this.candData.education.forEach((el: any) => {
      this.educationElements += `<li class='innerpadding' style='line-height: 30px; padding: 0 30px;'>
          <div class='companydata'>
              <h4 style='font-weight: normal;font-size: 22px;font-family: Jost, sans-serif;margin-bottom: 5px;'>${el.degreeName} | ${el.degreeType} <span class='years float-right' style='font-size: 16px;color: #969EB5;float: right!important;font-weight: normal;'>${el.degreeCompletionDate.substring(0, 4)}</span>
              </h4>
              <span class='subheading' style='color: #969EB5;'>${el.schoolName}</span>
          </div>
      </li>`;
    });
    ...
    this.candData.certficationList.forEach((el: any) => {
      this.certificateElements += `<li class='innerpadding' style='line-height: 30px; padding: 0 30px;'>
          <div class='companydata'>
              <h4 style='font-weight: normal;font-size: 22px;font-family: Jost, sans-serif;margin-bottom: 5px;'>${el.certName}<span class='years float-right' style='font-size: 16px;color: #969EB5;float: right!important;font-weight: normal;'>${el.completedYear.substring(0, 4)}</span>
              </h4>
              <span class='subheading' style='color: #969EB5;'>${el.authorityName}</span>
          </div>
      </li>`;
      // certValue: "License"
    });
    ...
    this.candData.trainingList.forEach((el: any) => {
      this.TrainingElements += `<li class='innerpadding'>
          <div class='companydata'>
              <h4>${el.institutionName}<span class='years float-right'>${el.completedYear.substring(0, 4)}</span>
              </h4>
    ...
    this.candData.socialsLinks.forEach((el: any) => {
      this.socialsLinksElements += `<li style='line-height: 30px; margin-top: 5px; line-height: 35px;'>
          ...
          <a href='${el.socialURL}' style='color:#111111;text-decoration:none;display:inline-block;margin-left: 10px;'>${el.socialValue}</a>
      </li>`;
    });
                        ...
                        <div class='col-6'
                            ...
                            <div class='row d-flex align-items-center' style='display: -ms-flexbox;display: flex;-ms-flex-wrap: wrap;flex-wrap: wrap;margin-right: -15px;margin-left: -15px;display: -ms-flexbox !important;display: flex !important;-ms-flex-align: center !important;align-items: center !important;'>
                                ...
                                <div class='col-8' style='position: relative;padding-right: 15px;padding-left: 15px;max-width: 66.666667%;'>
                                    <div class='candata'>
                                        <h2 style='font-family: Jost, sans-serif;margin-bottom:0'>${this.candData.firstName} ${this.candData.lastName}
                                        </h2>
                                        <ul class='mb-0' style='font-family: Open Sans, sans-serif;margin-bottom: 0 !important;margin-top: 0;list-style-type: none;padding: 0;'>
                                            ...
                                            <li style='line-height: 30px;'><i
                                                    class='fa fa-map-marker-alt'></i> ${this.candData.location}</li>
                                            <li style='line-height: 30px;'>
    ...
    return allEles.replace(/(\r\n|\n|\r)/gm, "").replace(/<br>/g, '<br></br>').replace(/&/g, ' ');
  }
}
