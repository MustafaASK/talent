  ...
  loadData(obj: any) {
    ...
    this.candData.empHistory.forEach((el: any) => {
      ...
      this.empHistoryElements += `<li class='innerpadding emprecord'>
          <div class='companydata w100 fl'>
            <h4 class='fl'>${el.jobTitle} | ${el.companyName}</h4> <span class='subheading fr'>${el.startDate.substring(0, 4)} - ${el.currentCompany ? 'Present' : el.endDate.substring(0, 4)}</span>
          </div>
    ...
    this.candData.certficationList.forEach((el: any) => {
      this.certificateElements += `<li class='innerpadding'>
          <div class='companydata'>
              <h4>${el.certName}<span class='years float-right'>${el.completedYear.substring(0, 4)}</span>
              </h4>
    ...
    this.candData.skills.forEach((el: any) => {
      ...
      <td class="skillrating">
          <div class="skillDot grey"></div>
          <div class="skillDot grey"></div>
          <div class="skillDot blue"></div>
          <div class="skillDot blue"></div>
      </td>
  </tr>
          `;
    ...
    this.candData.socialsLinks.forEach((el: any) => {
      this.socialsLinksElements += `<li>
          ...
          <a href='${el.socialURL}'>${el.socialValue}</a>
      </li>`;
    });
    ...
    * {
    ...
    <body style='padding: 0 5px;background-color: #f3f5f7;'>
      ...
      <div class='fl' style='width: 150px; height: 150px;      border: 1px solid #D1D5E1; overflow: hidden; border-radius: 70px;      text-align: center;
      background-image: url(${this.candData.profilePicPath})'>
           </div>
           <div class='w60' style='padding-right: 15px;padding-left: 15px;margin: 40px 0 0 10px;display: inline-block;width: 45%;'>
               <h2 style='font-family: Jost, sans-serif; margin-bottom: 0'>
               ${this.candData.firstName} ${this.candData.lastName} </h2>
               <ul class='mb-0'>
                   <li>
                       <i class='fa fa-briefcase'></i>${this.candData.jobTitle}
                   </li>
                   <li><i class='fa fa-map-marker-alt'></i> ${this.candData.location}</li>
                   <li>
                       <span class='hire tag' style='background-color: #E5F9E7;border: none; color: #111;padding: 5px 15px;
                       margin-top: 5px; display: inline-block; line-height: 15px;
           ...
           <div class='w40' style='display: inline-block;position: relative;top: -70px;'>
               <p style='line-height: 0;text-align: right;'>${this.candData.phoneNo}</p>
      ...
      `+ (this.candData.careersummary ? `
        ...
        <div class='bb card' style='margin: 10px auto 0 5px; overflow: hidden; '>
            <section id='Certifications'>
                <h3 class='innerpadding'>Certifications</h3>
