...
export class ViewProfileComponent implements OnInit {
  ...
  getreviewdetails() {
    ...
    this.userService.getreviewdetails().subscribe((response: any) => {
      ...
      if (response && response.Status) {
        ...
        // if (response.assessmentsReportsList && response.assessmentsReportsList.length) {
        //   this.assessmentsReportsList = true;
        // }else{
        //     this.assessmentsReportsList = false;
        // }
  ...
  loadInItData(obj: any) {
    ...
    this.profileImageBase64 = this.domSanitizer.bypassSecurityTrustResourceUrl((this.candData.profilePicPath) ? (this.candData.profilePicPath).toString() : '');
    // setTimeout(() => {
    //   this.generatePDF();
    // }, 500);
  }
